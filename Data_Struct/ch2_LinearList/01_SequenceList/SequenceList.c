...
Status ListInsert_Sq(SqList *L, int i, LElemType_Sq e)
{
	...
	if(i < 1 || i > (*L).length + 1)      // length + 1 表示的是紧随队尾后的位置
	{
		return ERROR;
	}

	newbase = (LElemType_Sq*)realloc((*L).elem, ((*L).listsize+LISTINCREMENT)*sizeof(LElemType_Sq)); /* realloc用法 */
	if(!newbase)
		exit(OVERFLOW);
